...
These follow-up source changes are not present in the already-deployed contracts at the recorded addresses. Production remains unchanged pending explicit deployment approval for this follow-up release. The current deployed release remains the previously verified live release; the queue and lifecycle fixes require a new deployment before they can be claimed as live. **NOT READY FOR THE FOLLOW-UP RELEASE** until that approved deployment, live lifecycle flow, and frontend publication are completed.
